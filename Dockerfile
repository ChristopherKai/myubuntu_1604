...
RUN apt update && apt install -y openssh-server git locales ruby tmux sudo && rm -rf /var/lib/apt/lists/* \
    && sed -i "s|#PermitRootLogin prohibit-password|PermitRootLogin yes|g"  /etc/ssh/sshd_config && locale-gen en_US.UTF-8 
RUN easy_install pip && pip install --upgrade pip &&　pip install --no-cache-dir pwntools capstone ropgadget formatStringExploiter ipython  && gem install one_gadget
