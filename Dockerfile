...
RUN apt update && apt install -y python3 python3-pip openssh-server git locales ruby tmux sudo ruby-dev && rm -rf /var/lib/apt/lists/* \
    && sed -i "s|#PermitRootLogin prohibit-password|PermitRootLogin yes|g"  /etc/ssh/sshd_config && locale-gen en_US.UTF-8 
RUN  pip3 install --upgrade pip &&　pip3 install --no-cache-dir pwntools capstone ropgadget formatStringExploiter ipython  && gem install one_gadget seccomp-tools
